...
member_page_init = member_page_home_del |> 
  ...
  select(-site_length, -name_length, -pickup_detail_v1, -pickup_detail_v2) |> 
  mutate(concat_member = paste0(first.name, "||", last.name, "||", email)) |> 
  select(last_modified_date = last.modified.date, sign_up = sign.up, subscription, season
...
payments_init = payments_home_del |> 
  ...
  select(-site_length, -name_length, -pickup_detail_v1, -pickup_detail_v2) |> 
  mutate(concat_member = paste0(first.name, "||", last.name, "||", email)) |> 
  select(date...time, season, type, date.of.payment, amount, notes, file_date, concat_member) 
...
write.csv(payments_anon
          , paste0(repo_location, 'payments/', payments_date, '__payments.csv')
          , row.names = FALSE)
```
