...
# main_loc = 'C:/Users/rmarg/OneDrive/Documents/R/_ds_fairshares/Data/Farmigo_Database/step_1_anonymization/Raw/'
...
member_page_folder = paste0(main_loc, 'member_page/') 
payments_folder = paste0(main_loc, 'payments/') 
...
This is different from the Member Information above. This includes extra information like Balance amounts.

```{r}
file_master_member_page = list.files(member_page_folder) |> 
  as_tibble() |> 
  rename(file = value) |>
  mutate(file_full = paste0(member_page_folder,file)
         , snapshot_date = ymd(str_sub(file,0,10))
         ) |> 
  arrange(snapshot_date) |> 
  rownames_to_column() |> 
...
## join files to FS dates
file_master_member_page_fs = file_master_member_page |> 
  full_join(sr_wks, by = character()) |> 
  filter(snapshot_date >= fs_date_start & snapshot_date <= fs_date_end) |> 
  group_by(fs_date) |> 
  arrange(fs_date, desc(snapshot_date)) |> 
...
## error check to make sure only one file selected
stopifnot((file_master_member_page_fs |> 
             group_by(fs_date) |> 
             summarise(n = n()) |> 
             filter(n > 1) |> 
             pull()
...
## takes a bit to run so best to leave as separate table to test/run next few transformations
farmigo_member_page_orig = file_master_member_page_fs |> 
  left_join(file_master_member_page |> 
              # filter(rowname_itm %in% c(3)) |> 
              pull(file_full) |> 
              map_df(~read_csv(.x), .id = "rowname_itm") |> 
              rename_all(tolower) |> 
              select_all(list(~gsub(" ", "_", .)))
...
farmigo_member_page_history = farmigo_member_page_orig |> 
  select(date = fs_date, season, season_week, snapshot_date, member_id
         , last_modified_date, sign_up, subscription, season_member_page
         , pickup_site, city, state, zip
         , balance, payment, issues, status
         , route_name, payment_plan, discount
         , future_pending_payments, future_deliveries
         , first_delivery_date, last_delivery_date
         , coupon_id)

# head(farmigo_member_page_history)
```
...
## THIS PROCESS WORKS BUT THERE SEEMS TO BE A LOT OF DATA ISSUES. I NEED TO DO SOME FURTHER CHECKS ON SOME MORE RECENT WEEKS TO FIGURE OUT WHAT IS GOING ON. I THINK PART OF THE ISSUE IS HOW WE HANDLE CONVERSION OF THE CHARACTER VALUES TO INTEGERS.

farmigo_boxbuilder_orig = file_boxbuilder |>
  # filter(rowname_del %in% c(4,5)) |>
  pull(file_full) |>
  map_df(~bind_rows(cbind(read_csv(.x, col_names = FALSE) |> 
           rownames_to_column() |> 
           filter(rowname == 1) |> 
           select(-rowname) |> 
           pivot_longer(cols = starts_with("X"), names_to = "column_value", values_to ="date") 
           ,read_csv(.x, col_names = FALSE) |> 
                        rownames_to_column() |> 
                        filter(rowname == 2) |> 
                        select(-rowname) |> 
                        pivot_longer(cols = starts_with("X"), names_to = "column_value", values_to ="item") |> 
                        select(-column_value)
                      )  |> 
           mutate(concat_item = ifelse(is.na(date), item, paste(date, item, sep = "|||"))) |> 
           select(column_value, concat_item) |> 
           pivot_wider(names_from = column_value, values_from = concat_item ),
           read_csv(.x, col_names = FALSE) |> 
           rownames_to_column() |> 
           filter(rowname > 2) |> 
           select(-rowname)
...
# 
# 
# test_4 = farmigo_boxbuilder_orig |> 
#     filter(rowname_del == 1) |>
#     filter()
# 
# 
#   new_column_names <- farmigo_boxbuilder_orig |> 
#     filter(rowname_del == 2) |>
#     slice(1) |>   # Select the first row
#     as.character() # Convert to a character vector
# 
#   new_column_names = replace_na(new_column_names, "remove_variable")
#   
#   df_renamed <- farmigo_boxbuilder_orig |> 
#     filter(rowname_del == 2) |>
#     slice(-1)   # Remove the first row (the header row)
#   
#    names(df_renamed) = new_column_names
#   
#    pivot_df = df_renamed |> 
#     select(-remove_variable) |> 
#     rename(rowname_del = 2) |> 
#     pivot_longer(!c('rowname_del', 'Item','Unit'), names_to = "date_subscription", values_to = "count") |> 
#     mutate(count = as.integer(count)) |> 
#     filter(count > 0 & !is.na(Item) & item != "_") |> 
#     separate_wider_delim(date_subscription, delim = "|||", names = c("date","subscription")) |> 
#     mutate(date = mdy(date)) |> 
#     arrange(date, subscription, Item) |> 
#     rename(item = Item, unit = Unit)


files = farmigo_boxbuilder_orig |> select(rowname_del) |> distinct() |>  pull()

column_names = c("rowname_del", "item", "unit", "date","subscription", "count")

farmigo_boxbuilder_orig_transposed = tibble(
  !!!setNames(rep(list(character()), length(column_names)), column_names)
)

for (i in files) {
  
  new_column_names <- farmigo_boxbuilder_orig |> 
    filter(rowname_del == i) |>
    slice(1) |>   # Select the first row
    as.character() # Convert to a character vector
  ...
  if("remove_variable" %in% new_column_names) {
     df_renamed = df_renamed |> 
       select(-remove_variable)
  }
  
  pivot_df = df_renamed |> 
    rename(rowname_del = i) |> 
    pivot_longer(!c('rowname_del', 'Item','Unit'), names_to = "date_subscription", values_to = "count") |> 
    mutate(count = as.integer(count)) |> 
    filter(count > 0 & !is.na(Item)) |> 
    separate_wider_delim(date_subscription, delim = "|||", names = c("date","subscription")) |> 
    mutate(date = mdy(date)) |> 
    arrange(date, subscription, Item) |> 
    rename(item = Item, unit = Unit)
  ...
  farmigo_boxbuilder_orig_transposed = rbind(farmigo_boxbuilder_orig_transposed, pivot_df)
}

test = farmigo_boxbuilder_orig_transposed |> 
  distinct()

##### DID NOT GET PAST THIS POINT YET SO THE CODE BELOW HAS NOT BEEN CHECKED AT ALL.
...
inv_upld = inv_upld_all_itm |> 
  ...
  union_all(
    ...
    inv_upld_all_itm |> 
        ...
        mutate(upload = ifelse(!is.na(upload), upload, available)) 
    ##original logic
    # file_master_inv |> 
    #     filter(file_status == 'upload file' & 
...
farmigo_all_item_cnt <- farmigo_all_item |> 
             summarise(n = n()) |> 
             pull()

farmigo_all_item_pre_s19 <- farmigo_all_item |> 
  filter(date <= '2026-03-01')

farmigo_all_item_pre_s19_cnt <- farmigo_all_item_pre_s19 |> 
             summarise(n = n()) |> 
             pull()

farmigo_all_item_s19_forward <- farmigo_all_item |> 
  filter(date >= '2026-03-01')

farmigo_all_item_s19_forward_cnt <- farmigo_all_item_s19_forward |> 
             summarise(n = n()) |> 
             pull()

stopifnot((farmigo_all_item_cnt) ==  
            (farmigo_all_item_pre_s19_cnt
             +
             farmigo_all_item_s19_forward_cnt)
          )
```
...
```{r}
file_master_payments = list.files(payments_folder) |> 
  as_tibble() |> 
  rename(file = value) |>
  mutate(file_full = paste0(payments_folder,file)
         , first_dash = str_locate(file,'-')[1]
         , start_date = mdy(str_sub(file, first_dash+1, first_dash+11))
         , end_date = mdy(str_sub(file, first_dash+12, first_dash+23))
         , snapshot_date = ymd(str_sub(file,0,10))
...
file_master_payments_page = file_master_payments |> 
  full_join(sr_wks, by = character()) |> 
  filter(fs_date >= start_date & fs_date <= end_date) |> 
  group_by(fs_date) |> 
  arrange(fs_date, desc(snapshot_date)) |> 
...
## error check (cannot have more than one file week associated with a week of data)
stopifnot((file_master_payments_page |> 
             filter(file_rank == 1) |> 
             group_by(fs_date) |> 
             summarise(n = n()) |> 
             filter(n > 1) |> 
...
## takes a bit to run so best to leave as separate table to test/run next few transformations
farmigo_payments_orig = file_master_payments |> 
  left_join(file_master_payments |> 
              pull(file_full) |> 
              map_df(~read_csv(.x), .id = "rowname_del")
            , by = c('rowname_del')
  )

farmigo_payments_orig_join = farmigo_payments_orig |> 
  mutate(date_time_short = str_sub(date_time,0,str_locate(date_time, " ")[,1])
         , time_short = trimws(str_sub(date_time,str_locate(date_time, " ")[,1],length(date_time)))
         , date_time_new = mdy_hm(paste(date_time_short,time_short))
         , date_of_payment_new_a = dmy(date_of_payment)
         , date_of_payment_new_b = mdy(date_of_payment)
         , date_of_payment_new = case_when(is.na(date_of_payment_new_a) ~ date_of_payment_new_b
                                           , TRUE ~ date_of_payment_new_a)
         ) |> 
  select(rowname_del, file, file_full, start_date, end_date, snapshot_date
           , date_time = date_time_new, season_payments, type, date_of_payment = date_of_payment_new
           , amount, notes, file_date, member_id) |> 
  inner_join(file_master_payments_page |> 
               filter(file_rank == 1) |> 
               select(rowname_del, snapshot_date
                      , fs_date, fs_date_string, fs_date_start, fs_date_end, season, season_week)
             , by = c('rowname_del','snapshot_date')
  ...
  relocate(fs_date, fs_date_string, fs_date_start, fs_date_end) |> 
  filter(date_time >= fs_date_start & date_time <= fs_date_end)

# farmigo_payments_orig_join = farmigo_payments_orig |> 
#   filter(snapshot_date <= '2025-08-22') |> 
#   mutate(date_time_short = str_sub(date_time,0,str_locate(date_time, " ")[,1])
#          , time_short = trimws(str_sub(date_time,str_locate(date_time, " ")[,1],length(date_time)))
#          , date_time_new = mdy_hm(paste(date_time_short,time_short))
#          , date_of_payment_new = dmy(date_of_payment)
#          ) |> 
#   select(rowname_del, file, file_full, start_date, end_date, snapshot_date
#          , date_time = date_time_new, season_payments, type, date_of_payment = date_of_payment_new
#          , amount, notes, file_date, member_id) |> 
#   rbind(t = farmigo_payments_orig |> 
#     filter(snapshot_date > '2025-08-22') |> 
#     mutate(date_time_short = str_sub(date_time,0,str_locate(date_time, " ")[,1])
#            , time_short = trimws(str_sub(date_time,str_locate(date_time, " ")[,1],length(date_time)))
#            , date_time_new = ymd_hm(paste(date_time_short,time_short))
#            , date_of_payment_new = mdy(date_of_payment)
#            ) |> 
#     select(rowname_del, file, file_full, start_date, end_date, snapshot_date
#            , date_time = date_time_new, season_payments, type, date_of_payment = date_of_payment_new
#            , amount, notes, file_date, member_id)
...
## error check to make sure a week is not being duplicated by multiple files
stopifnot((farmigo_payments_orig_join |> 
             select(fs_date, rowname_del) |> 
             distinct() |> 
             group_by(fs_date) |> 
             summarise(n = n()) |> 
...
farmigo_payments = farmigo_payments_orig_join |> 
  select(date = fs_date, season, season_week, snapshot_date
         , date_time, member_id, season_payments, type, date_of_payment
         , amount, notes)
...
# member page
cell_count_del_member_page = dim(farmigo_member_page_history)[1]*dim(farmigo_member_page_history)[2]

print(paste0('percentage filled to Google Sheets maximum cell count (Member Page) ',(cell_count_del_member_page / 10000000)*100, '%'))

# payments
cell_count_payments = dim(farmigo_payments)[1]*dim(farmigo_payments)[2]

print(paste0('percentage filled to Google Sheets maximum cell count (Payments) ',(cell_count_payments / 10000000)*100, '%'))

cell_count_farmigo <- cell_count_member_history + cell_count_memb_del + cell_count_all_subscr + cell_count_del_aggr + cell_count_del_member_page + cell_count_payments

print(paste0('percentage filled to Google Sheets maximum cells (Farmigo Data 1) ',(cell_count_farmigo / 10000000)*100, '%'))

stopifnot(cell_count_farmigo <= 10000000)
...
# all items - s19 forward
cell_count_all_item_s19_forward = dim(farmigo_all_item_s19_forward)[1]*dim(farmigo_all_item_s19_forward)[2]

print(paste0('percentage filled to Google Sheets maximum cell count (All Items s19 forward) ',(cell_count_all_item_s19_forward / 10000000)*100, '%'))

cell_count_farmigo_all_item <- cell_count_all_item + cell_count_all_item_s19_forward

print(paste0('percentage filled to Google Sheets maximum cells (Farmigo Data All Item) ',(cell_count_farmigo_all_item / 10000000)*100, '%'))

stopifnot(cell_count_farmigo_all_item <= 10000000)
...
```{r}
sheet_id_farmigo = "1W34-fLMAWPt5VvWb6kiVxWPikgn0Ly2e6JjhQIOmsF4" #original source
# sheet_id_farmigo = "1c6KxrIaysQPnSsAqWCMxBkKXAWvgiWw57V2VhUdMI5c" #personal
sheet_id_farmigo_all_item = "16MCzzQ8cEGdHOI0u3dO8Gzr6t1aa0PedvF0lssyoJTI" #original source
```
...
# member page
sheet_write(farmigo_member_page_history, sheet_id_farmigo, sheet = "member_page")

# payments
sheet_write(farmigo_payments, sheet_id_farmigo, sheet = "payments")

# all items
# sheet_write(farmigo_all_item, sheet_id_farmigo_all_item, sheet = "all_items")
# sheet_write(farmigo_all_item_pre_s19, sheet_id_farmigo_all_item, sheet = "all_items_pre_s19")
sheet_write(farmigo_all_item_s19_forward, sheet_id_farmigo_all_item, sheet = "all_items_s19_forward")
