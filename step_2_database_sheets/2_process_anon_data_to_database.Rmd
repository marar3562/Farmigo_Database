...
##### LOTS OF ISSUES WITH DATES HERE... NEED TO FIGURE THIS OUT.
farmigo_payments_orig_join = farmigo_payments_orig |> 
  mutate(date_time_pull = str_replace_all(str_sub(date_time,0,10),"/","-")
         , date_time_1 = mdy(str_sub(date_time,0,10))
         , date_time_2 = mdy(str_sub(date_time,0,8))
         , date_time_3 = ymd(date_time_pull)
         ) |> 
...
farmigo_payments = farmigo_payments_orig_join |> 
  select(date = fs_date, season, season_week, snapshot_date, delivery_date, member_id, ...)
  # select(-file, -file_full, -start_date, -end_date) 
