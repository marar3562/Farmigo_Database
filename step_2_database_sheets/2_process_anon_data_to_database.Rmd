...
## takes a bit to run so best to leave as separate table to test/run next few transformations
test_1 = file_boxbuilder |>
  filter(rowname_del %in% c(4,5)) |>
  pull(file_full) |>
  map_df(~read_csv(.x, col_names = FALSE) , .id = "rowname_del") 


test_2 = file_boxbuilder |>
  filter(rowname_del %in% c(4,5)) |>
  pull(file_full) |>
  map_df(~read_csv(.x, col_names = FALSE) |> 
           rownames_to_column() |> 
           filter(is.na(X1) | X1 == 'Item') |> 
           
         , .id = "rowname_del") 


test_3 = file_boxbuilder |>
...
test_3 |> select(`rowname...1`)

test_10 = test[1:nrow(test)-1,] |> 
  rownames_to_column() |> 
  filter(is.na(X1) | X1 == 'Item') |> 
  group_by(rowname) |> 
